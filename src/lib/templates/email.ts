...
export const EmailTemplates = {
  ...
  // * Doctor Appointment Cancellation Notification
  cancelAppointment(params: {
    ...
    const title = `Appointment with patient ${params.patientId} has been cancelled`;
    const formattedDate = new Date(params.dateTime).toLocaleString();
    const cancelledBy =
      params.cancelledBy === 'doctor'
        ? 'You have cancelled this appointment.'
        : params.cancelledBy === 'patient'
          ? 'The patient has cancelled this appointment.'
          : 'The system automatically cancelled this appointment.';

    const content = `
    <h2 class="greeting">Hello Dr. ${params.name},</h2>
    <p class="message">${cancelledBy}</p>
    <div style="background:#fff3f3;border-left:4px solid #dc3545;padding:16px 20px;margin:20px 0;border-radius:0 8px 8px 0;">
      <p style="color:#555;font-size:14px;margin:0;">
        <strong>Patient:</strong> ${params.patientId}<br/>
      ...
      ${
        params.reason
          ? `<p style="margin-top:12px;color:#dc3545;font-size:14px;"><strong>Reason:</strong> ${params.reason}</p>`
          : ''
      }
    </div>
    <p class="message">You can view appointment history or reschedule another session below.</p>
    <a href="${params.appointmentLink}" class="cta-button">View in Dashboard</a>
  `;

    const text = `Appointment with patient ${params.patientId} on ${formattedDate} has been cancelled. ${
      params.reason ? `Reason: ${params.reason}. ` : ''
    }Check details: ${params.appointmentLink}`;
